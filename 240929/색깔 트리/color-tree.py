...
def check_make_child(cur_node, depth):
    if cur_node.parent == -1:
        return True  # No parent means root node, stop the recursion
...
def add_node(m_id, p_id, color, max_depth):
    if p_id == -1:
        check_root[m_id] = PARENT  # Mark this node as a root
    if check_root[m_id] == PARENT or (p_id in nodes and check_make_child(nodes[p_id], 1)):
        nodes[m_id] = Node(m_id, color, max_depth, -1 if check_root[m_id] == PARENT else p_id)
